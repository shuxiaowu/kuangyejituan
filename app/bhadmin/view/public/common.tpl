...
<header id="navbar" style="position:fixed;">
    <div id="navbar-container" class="boxed">
        <div class="navbar-header">
           <a href="{:url('index/index')}" class="navbar-brand"><img src="__img__/logo/bhlogo.png" alt="{:config('devcompany')}" class="brand-icon">
        ...
        <div class="navbar-content clearfix">
            <ul class="nav navbar-top-links pull-left">
                            ...
                            <div class="nano-content">
                                <ul class="head-list">
                                  <if condition="$adminsetting['c_site'] eq 1">
                                    <li>
                                      <a class="media" href="{:url('system/sysmod')}"><div class="media-left"><i class="pli-computer-secure icon-2x"></i></div>
                                  ...
                                  <if condition="$adminsetting['loginwarning'] neq ''">
                                    <li>
                                      <a class="media" href="{:url('index/modpass')}"><div class="media-left"><i class="pli-add-user-plus-star icon-2x"></i></div>
                                         <div class="media-body"><div class="text-nowrap">登录提示</div><small class="text-muted">{:strip_tags($adminsetting['loginwarning'])}</small></div>
                                      </a>
                                    </li>
                                  </if>
                                  <if condition="$adminsetting['passWarning'] neq ''">
                                    <li>
                                      <a class="media" href="{:url('index/modpass')}"><div class="media-left"><i class="pli-add-user-plus-star icon-2x"></i></div>
                                         <div class="media-body"><div class="text-nowrap">登录密码提示</div><small class="text-muted">{:strip_tags($adminsetting['passWarning'])}</small></div>
                                      </a>
                                    </li>
                                  </if>
                              ...
                              <table class="table table-hover" width="100%" style="margin:0px auto;">
                               <tr class="active"><td style=" border:none;">&nbsp;数据库</td><td width="100" align="center" style=" border:none;">备份状态</td></tr>
                                <volist name="adminback['tablist']" id="tobj">
                                  <tr><td>&nbsp;{$tobj}</td><td align="center"><span class="tab-state tabstate{$tobj}" data-table="{$tobj}"><i class="fa fa-spinner fa-spin" title="备份中..."></i></span></td></tr>
                                </volist>
                              </table>
                            </div>
                        </div>
                        <div class="pad-all bord-top"><a href="{:url('System/databackuplist')}" class="btn-link text-dark box-block"><i class="fa fa-angle-right fa-lg pull-right"></i>查看备份详情</a></div>
                     </div>
                </li>
                ...
                <if condition="$cmenu neq '' and $admin_menu eq 1">
                  <volist name="cmenu" id="cobj">
                    <li <if condition="$topmenuid eq $cobj['Id']">class="menuselect"</if>><a href="javascript:void(0)" class="menu-uri" data-id="{$cobj['Id']}">{$cobj['title']}</a></li>
                  </volist>
                </if>
                <if condition="$admin_menu eq 0">
            ...
            <ul class="nav navbar-top-links pull-right">
                ...
                <li><a href="{:url('index/cleancache')}"><span class="pull-right"><i class="pli-recycling" style="font-size:1.5em; line-height:40px;"></i></span><div class="username hidden-xs">清除缓存</div></a></li>
                <li id="dropdown-user" class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-xs dropdown-menu-right panel-default">
                        <ul class="head-list">
                            <li class="bord-btm"><a href="{:url('index/index')}"><i class="pli-home icon-lg icon-fw"></i> 系统首页</a></li>
                            <li class="bord-btm"><a href="{:url('index/modpass')}"><i class="pli-gear icon-lg icon-fw"></i> 修改密码</a></li>
                        ...
                        <div class="pad-all text-right"><a href="{:url('index/logout')}" class="btn btn-primary"><i class="pli-unlock"></i> 退出</a></div>
                    </div>
                </li>
            </ul>
        </div>
...
<div class="boxed">
 ...
 <div id="content-container">
   <div id="page-content"><div class="panel" style="padding:0; overflow:auto; "><block name="main"></block></div></div>
 </div>
                   ...
                   <ul id="mainnav-menu" class="list-group" style="margin:0;">
                      <volist name="menu" id="cobj">
                       <in name="cobj['Id']" value="$myauth">
                         <li class="list-header <if condition="$cobj['isopen'] eq 0">list-hide</if>" data-id="{$cobj['Id']}" style="font-weight:normal; font-size:14px; cursor:pointer;">{$cobj['title']} <span class="pull-right badge badge-warning hide" style="font-size:12px;">{:count($cobj['menu'])}</span></li>
                          <if condition="$cobj['menu'] neq ''"> 
                             <volist name="cobj['menu']" id="mobj">
                              <in name="mobj['Id']" value="$myauth">
                                  <li class="levone lione{$mobj['Id']} lio{$cobj['Id']}" data-isopen="{$mobj['isopen']}"><!--active"-->
                                   <a href="{:tplUrl($mobj['linkurl'])}">
                                   <if condition="$mobj['icon'] neq ''">
                                    {$mobj['isext']?faicon($mobj['icon'],'i'):icon($mobj['icon'],'i')}
                                   <else/>
                                    <i class="pli-computer-secure"></i>
                                   </if>
                                   <span class="menu-title">{$mobj['title']}</span><i class="arrow"></i></a>
                                    <if condition="$mobj['smenu'] neq ''">
                                     <ul class="collapse">
                                       <volist name="mobj['smenu']" id="sobj">
                                        <in name="sobj['Id']" value="$myauth">
                                         <li class="levtwo litwo{$sobj['Id']}"><a href="{:tplUrl($sobj['linkurl'])}">{$sobj['title']}</a></li>
                                        </in>
                                       </volist>
                                     </ul>
                                    </if>
