...
<block name="main">
 <div class="pubmain">
  <div class="panel-heading">
   <h3 class="panel-title pull-left">{$systitle}管理</h3>
   <div class="pull-right mar10">{:btn(array('vals'=>'添加'.$systitle,'size'=>3,'icon'=>'plus','scene'=>'primary','url'=>url('website/linksadd','sty='.$sty.'&tables='.$dshow['table'])))}</div>
  </div>
 ...
 <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="{:tabstyle()}">
   <tr class="active">
     ...
     <td width="85" align="center" valign="middle">{$subtitle}排序</td>
   ...
   <tr class="maintr">
    ...
    <td align="center" valign="middle">{:btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','round'=>1,'url'=>url('website/linksmod','id='.$obj['Id'].'&tables='.$dshow['table'])))}</td>
   </tr>
   </volist>
   <tr>
   ...
   <else/>
    <div class="alert alert-danger">暂无数据。</div>
   </if>
 </div>
 </div>
</block>